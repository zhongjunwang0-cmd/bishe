...
CREATE TABLE `sys_user` (
    ...
    `level` VARCHAR(20) DEFAULT 'B1' COMMENT '英语水平 A1/A2/B1/B2/CET-4/CET-6/IELTS/TOEFL',
    `target_exam` VARCHAR(20) DEFAULT 'GENERAL' COMMENT '目标考试 CET-4/CET-6/IELTS/TOEFL/GENERAL',
    `daily_goal` INT DEFAULT 30 COMMENT '每日学习目标（分钟）',
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP,
    `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE=InnoDB COMMENT='用户表';

CREATE TABLE `biz_learning_plan_item` (
    `id` BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
